...
export type EVOwnerInfoUser = UserTableUser & {
  phone?: string;
...
type EVOwnerInfoProps = {
  user: EVOwnerInfoUser;
};
...
export default function EVOwnerInfo({ user }: EVOwnerInfoProps) {
  const phone = user.phone ?? "+880 1712-345678";
  const totalBookings = user.totalBookings ?? 24;
  const totalPayments = user.totalPayments ?? "€1280";
  const lastLogin = user.lastLogin ?? "2 hours ago";
